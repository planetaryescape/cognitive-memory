...
| Semantic | 240 | 0.02 | Facts, preferences, relationships |
...
When a memory is retrieved, its stability increases. Direct matches get a larger boost (0.1) than associatively-activated memories (0.05). Both are modulated by a spaced repetition factor: longer gaps between retrievals produce bigger boosts.
...
Periodic maintenance (`tick()`) handles conflict resolution, cold migration, TTL expiry, and consolidation in the background. v0.5.1 source also includes [temporal reconstruction](/cognitive-memory/concepts/temporal-reconstruction/) metadata and routing for sequence questions, but that retrieval mode is experimental and off by default.
