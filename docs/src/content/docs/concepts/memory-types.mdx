...
**Decay behavior:** Base decay rate of 240 days in the v0.5 defaults. Facts persist longer than events because they remain relevant across time.
...
**Decay behavior:** Base decay rate of 120 days, with a **retention floor of 0.60**. This means a core memory's retention will never drop below 60%, no matter how long it goes without being accessed. Regular memories have a floor of 0.02.
...
2. **Through promotion:** Any memory that meets all three criteria is automatically promoted:
   ...
   - Accessed across `>= 2` distinct sessions
...
| Semantic | 240 | 0.02 | `MemoryCategory.SEMANTIC` |
...
Core memory behavior in TypeScript uses the same `category: "core"` value as Python. The default decay rates map directly:
...
The v0.5.1 source also carries experimental temporal metadata on each memory:

| Concept | Python | TypeScript | Purpose |
|---------|--------|------------|---------|
| Temporal metadata | `temporal` | `temporal` | Mentioned time, event time, validity status, raw time expressions, and temporal relations |
| Event frame | `event_frame` | `eventFrame` | Structured subject/action/object/location/status view of the memory |

This is separate from both `category` and `memory_type`. `category` controls decay, `memory_type` controls expiry behavior, and `temporal`/`event_frame` support default-off temporal reconstruction for sequence and current-state questions.
