...
| `associative_boost` | 0.05 | Linked memories strengthen more from co-retrieval |
...
| `core_session_threshold` | 2 | Fewer distinct sessions needed |
...
| Semantic | 240 | 0.02 | Fades slowly |
| Procedural | Infinity | 0.02 | Never decays |
| Core | 120 | 0.60 | Fades slowly, high floor |

You can override base decay rates through `base_decay_rates` in Python and `decayRates` in TypeScript. If you only want memories to last longer after use, tune `direct_boost` first; if the category itself is wrong for your domain, change the category decay rate explicitly.
